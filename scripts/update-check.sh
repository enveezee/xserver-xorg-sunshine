...
# Normalize Debian version to match tag format
# Convert ":" and "+" to "_"
NORMALIZED_DEBIAN_VERSION=$(echo "$DEBIAN_VERSION" | sed 's/[:+]/_/g')

echo "Normalized Debian version: $NORMALIZED_DEBIAN_VERSION"
...
if [ "$NORMALIZED_DEBIAN_VERSION" = "$LATEST_VERSION" ]; then
    echo "skip=true" >> "$GITHUB_OUTPUT"
    exit 0
fi
